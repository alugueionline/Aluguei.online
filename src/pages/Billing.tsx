                    ...
                    <TableRow key={bill.id} className="hover:bg-gray-50/50 transition-colors border-gray-50">
                      <TableCell className="p-6">
                        <div className="flex items-center gap-3">
                          <div className={cn("p-2.5 rounded-xl", bill.billing_method === 'consumo_kwh' ? 'bg-orange-50 text-orange-600' : bill.billing_method === 'por_pessoa' ? 'bg-blue-50 text-blue-600' : bill.type === 'aluguel' ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-50 text-slate-600')}>
                            {bill.type === 'energia' ? <Zap className="w-4 h-4" /> : bill.type === 'agua' ? <Droplets className="w-4 h-4" /> : <FileText className="w-4 h-4" />}
                          </div>
                          <div>
                            <span className="font-bold capitalize text-sm block">{bill.type}</span>
                            <span className="text-[10px] text-gray-400 font-bold uppercase block">
                              {bill.billing_method === 'consumo_kwh' ? 'Consumo kWh' : bill.billing_method === 'por_pessoa' ? `Rateio (${bill.residents} mor.)` : 'Fixo'}
                            </span>
                            {bill.description && (
                              <span className="text-[10px] text-blue-600 font-medium block mt-0.5 italic">
                                Obs: {bill.description}
                              </span>
                            )}
                          </div>
                        </div>
...
<dyad-write path="src/pages/Financial.tsx" description="Atualizando a tabela de transações financeiras para exibir a observação personalizada">
"use client";
  ...
  return (
    <DashboardLayout title="Gestão Financeira">
                          ...
                          <tr key={item.id} className={cn(
                            ...
                            <td className="p-6">
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  ...
                                  {isAtrasado && <span className="text-[9px] font-black text-rose-400 uppercase tracking-widest block">Atrasado</span>}
                                  {item.description && (
                                    <span className="text-[10px] text-blue-600 font-medium block mt-0.5 italic">
                            ...
                            <td className="p-6 text-right">
                              <div className="flex items-center justify-end gap-2">
                                {item.status !== 'pago' ? (
                                  <Button size="sm" variant="outline" onClick={() => handleMarkAsPaid(item.id)} className="h-9 rounded-xl border-emerald-100 text-emerald-600 hover:bg-emerald-50 font-bold text-xs">Baixar</Button>
                                ) : item.status === 'pago' && (
                                  <Button size="sm" variant="ghost" onClick={() => handleRevertPayment(item.id)} className="h-9 rounded-xl text-slate-400 hover:text-blue-600 hover:bg-blue-50 font-bold text-xs gap-1.5" title="Reverter"><RotateCcw className="w-3.5 h-3.5" /> Reverter</Button>
                                )}
                                <Button size="icon" variant="ghost" onClick={() => handleEditItem(item)} className="h-9 w-9 rounded-xl text-slate-300 hover:text-blue-600 hover:bg-blue-50"><Edit2 className="w-4 h-4" /></Button>
                                <Button size="icon" variant="ghost" onClick={() => handleDeleteItem(item)} className="h-9 w-9 rounded-xl text-slate-300 hover:text-rose-500 hover:bg-rose-50"><Trash2 className="w-4 h-4" /></Button>
                              </div>
                            </td>
                          </tr>
                        );
      ...
      <BillingModal 
        ...
        onSave={() => queryClient.invalidateQueries({ queryKey: ['bills'] })}
...
<dyad-chat-summary>Adicionando campo de observação/descrição nas faturas e transações</dyad-chat-summary>

Adicionei o campo de **Observação / Descrição** nos modais de faturamento e transações rápidas, e configurei o histórico de pagamentos e as tabelas financeiras para exibir essa observação personalizada de forma clara.<dyad-write path="src/pages/Financial.tsx" description="Re-escrevendo o arquivo Financial.tsx com a tag de fechamento correta para garantir que as alterações de observação sejam salvas.">
"use client";
